...
<div class="container-xxl flex-grow-1">
    <div class="d-flex justify-content-between align-items center">
        <h4 class="fw-bold mb-4"><span class="fw-light">ဝင်ငွေ - လွှဲငွေရ /</span>
            {{ __('template_names.list_title_text') }} </h4>

        <h5 class=" d-flex justify-content-end">
            <a href="{{ route('incomeCreate') }}" class="btn btn-primary">
    ...
    <div class="card sec-filter mb-2" @if (request()->filled('payment_date') ||
        ...
        <form action="{{ route('incomeList') }}" method="GET">
            <div class="row g-3 align-items-end">
                ...
                <div class="col-6 col-sm-12 col-lg-3">
                    ...
                    <input type="text" name="amount_in_kyat" class="form-control" value="{{ request('amount_in_kyat') }}">
                </div>

                {{-- Amount in Baht --}}
                <div class="col-6 col-sm-12 col-lg-3">
                    <label class="form-label mb-1">အမောင့် (ဘတ်)</label>
                    <input type="text" name="amount_in_baht" class="form-control" value="{{ request('amount_in_baht') }}">
                </div>

                {{-- Payment Method --}}
                <div class="col-6 col-sm-12 col-lg-3">
                    <label class="form-label mb-1">ငွေလွှဲအမျိုးအစား</label>
                    <input type="text" name="payment_method" class="form-control" value="{{ request('payment_method') }}">
                </div>
                ...
                <div class="col-12 col-sm-auto ms-sm-auto">
                    <div class="d-flex gap-2 flex-column flex-sm-row">
                        ...
                        <a href="{{ route('incomeList') }}" class="btn btn-primary w-100 w-sm-auto"
    ...
    <div class="card">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mt-3">
                <div class="mb-2">
                    ဝင်ငွေ - လွှဲငွေရစာရင်း စုစုပေါင်း <span class="ms-2">-</span> <span class="ms-2">
                        {{ $incomes->count() }} ခု</span>
                </div>
                ...
                <table class="table table-bordered table-hover trans-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="nowrap">
                                {{ $income->date ? \Carbon\Carbon::createFromFormat('Y-m-d',
                            ...
                            <td class="nowrap text-end">
                                {{ $income->amount_in_baht !== null ? remove_unvaluable_zero($income->amount_in_baht) :
                                '-' }}
                            </td>

                            <td class="nowrap text-end">
                                {{ $income->amount_in_kyat !== null
                                ? number_format($income->amount_in_kyat, floor($income->amount_in_kyat) ==
                                $income->amount_in_kyat ? 0 : 2)
                                : '-' }}
                            </td>
                            ...
                            <td class="wrap-text">{{ $income->remark ?? '-' }}</td>
                            <td class="nowrap">
                                <a class="btn btn-sm btn-primary mx-3"
                                    href="{{ route('incomeEdit', $income->income_id) }}"><i class="bx bx-edit-alt"></i>
                                </a>
                                <a class="btn btn-sm btn-danger show-delete-modal" data-bs-toggle="modal"
                                    data-bs-target="#delete_confirm" data-id="{{ $income->income_id }}"
                                    data-resource-name="income_id" data-url="{{ route('incomeDelete') }}"
                        ...
                        <tr>
                            <td colspan="7" class="text-center text-muted py-4">
                                ဒီလအတွင်း ဝင်ငွေ/လွှဲငွေရ စာရင်း မရှိသေးပါ
                            </td>
                        </tr>
                        @endforelse
                        @endif
